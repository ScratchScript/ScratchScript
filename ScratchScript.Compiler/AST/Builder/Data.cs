...
public partial class ScratchScriptVisitor
{
        ...
        ScratchScriptParser.VariableDeclarationStatementContext context)
    {
        ...
        var node = new IrSetCommandNode(name, expression).WithContext(context);

        _scope.Variables.Add(new ScratchScriptVariable(name));
        if (!LocationInformation.Variables.ContainsKey(_scope.Depth))
    ...
    public override IrNode? VisitAssignmentStatement(ScratchScriptParser.AssignmentStatementContext context)
    {
                ...
                { } argument)
        {
            if (argument.Type != DetermineExpressionType(expression))
            {
                ...
                DiagnosticReporter.Instance.Note((int)ScratchScriptNote.FunctionArgumentTypeSetAt, typeSetter, typeSetter);
                return null;
            }
            ...
            return Target.Function.HandleFunctionArgumentAssignment(_scope, name, expression);#1#
        }*/
        ...
        /*if (variable.Type != DetermineExpressionType(expression))
        {
            ...
            DiagnosticReporter.Instance.Error((int)ScratchScriptError.TypeMismatch, context, context.expression(), variable.Type,
                DetermineExpressionType(expression));
            DiagnosticReporter.Instance.Note((int)ScratchScriptNote.VariableTypeSetAt, locationInformation.Context,
