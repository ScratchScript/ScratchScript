...
public partial class ScratchIRVisitor
{
    public override object VisitBinaryAddExpression(ScratchIRParser.BinaryAddExpressionContext context) =>
        VisitMathematicalExpression(Visit(context.addOperators()), Visit(context.expression(0)),
            Visit(context.expression(1)));

    public override object VisitBinaryMultiplyExpression(ScratchIRParser.BinaryMultiplyExpressionContext context) =>
        VisitMathematicalExpression(Visit(context.multiplyOperators()), Visit(context.expression(0)),
            Visit(context.expression(1)));

    public override object VisitBinaryBooleanExpression(ScratchIRParser.BinaryBooleanExpressionContext context) =>
        VisitConditionExpression(Visit(context.booleanOperators()), Visit(context.expression(0)),
            Visit(context.expression(1)));

    public override object VisitBinaryCompareExpression(ScratchIRParser.BinaryCompareExpressionContext context) =>
        VisitConditionExpression(Visit(context.compareOperators()), Visit(context.expression(0)),
            Visit(context.expression(1)));

    private Block VisitMathematicalExpression(object? op, object? left, object? right)
    {
        if (op is not Block operatorBlock) throw new Exception("The operator block passed to VisitMathematicalExpression was not a block.");
    ...
    {
        if (op is not Block operatorBlock) throw new Exception("The operator block passed to VisitConditionExpression was not a block.");
